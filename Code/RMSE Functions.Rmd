...
# Error message: Error in .oci.Connect(.oci.drv(), username = username, password = password, : ORA-12520: TNS:listener could not find available handler for requested type of server
```{r}
assay1764_agents <- agent_distance_list(assay1764_sub)
assay1763_agents <- agent_distance_list(assay1763_sub)
assay1761_agents <- agent_distance_list(assay1761_sub)
```
...
assay1764_agents %>% image(main="Assay 1764 Agent Similarity")
assay1763_agents %>% image(main="Assay 1763 Agent Similarity")
assay1761_agents %>% image(main="Assay 1761 Agent Similarity")
dev.off()
```


# Correlation between Agent Distances and Synergy Distances
```{r}
pdf("MalariaAssayCorrelationAgentAndBliss_sub.pdf", width=14, height=5)
par(mfrow=c(1,3))
plot( as.vector(assay1764_syrjala_bliss_matrices) ~ as.vector(assay1764_agents), 
      pch=16, cex=0.05,
      main="Assay 1764 Subset Single Agent and Bliss",
      xlab="Distance of Single Agents", ylab="Syrjala Test- Delta Bliss Matrices" )
# make a linear model
assay1764_agent_lm <- lm(as.vector(assay1764_syrjala_bliss_matrices) ~ as.vector(assay1764_agents))
# adding it to the graphic
assay1764_agent_lm %>% abline(., col="red")

plot( as.vector(assay1763_syrjala_bliss_matrices) ~ as.vector(assay1763_agents), 
      pc=16, cex=0.05,
      main="Assay 1763 Subset Single Agent and Bliss",
      xlab="Distance of Single Agents", ylab="Syrjala Test- Delta Bliss Matrices" )
# make a linear model
assay1763_agent_lm <- lm(as.vector(assay1763_syrjala_bliss_matrices) ~ as.vector(assay1763_agents))
# adding it to the graphic
...
plot( as.vector(assay1761_syrjala_bliss_matrices) ~ as.vector(assay1761_syrjala_response_matrices), 
      ...
      xlab="Distance of Single Agents", ylab="Syrjala Test- Delta Bliss Matrices" )
# make a linear model
assay1761_agent_lm <- lm(as.vector(assay1761_syrjala_bliss_matrices) ~ as.vector(assay1761_agents))
# adding it to the graphic
...
assay1764_sub_last <- good_subset(assay1764, assay1764_good, keep_last = TRUE)
